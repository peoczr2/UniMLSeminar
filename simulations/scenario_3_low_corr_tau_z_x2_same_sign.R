run_scenario_3_low_corr_tau_z_x2_same_sign <- function(repo_root, output_dir, n_train, n_test, num_trees, beat_penalty, target_share, seed, methods_to_run = NULL) {
  set.seed(seed)
  ...
  if ("BTGQ" %in% result_names) {
    btgq_plot_path <- file.path(output_dir, sprintf("scenario_3_low_corr_tau_z_x2_same_sign_btgq_lambda_trace_seed_%s.png", seed))
  ...
  if ("BTGQ_VALID" %in% result_names) {
    btgq_valid_plot_path <- file.path(output_dir, sprintf("scenario_3_low_corr_tau_z_x2_same_sign_btgq_valid_lambda_trace_seed_%s.png", seed))
    plot_btgq_lambda_trace(
  ...
  if ("BTGQ_MAX" %in% result_names) {
    btgq_max_plot_path <- file.path(output_dir, sprintf("scenario_3_low_corr_tau_z_x2_same_sign_btgq_max_lambda_trace_seed_%s.png", seed))
    plot_btgq_lambda_trace(
      method_results[[which(result_names == "BTGQ_MAX")[1]]]$lambda_trace,
      btgq_max_plot_path,
      "Scenario 3 BTGQ_MAX: Lambda vs Targeted Group Demo",
      target_quota = 0.5,
  ...
  if (length(method_results) > 0) {
    ...
    plot_path <- file.path(output_dir, "scenario_3_low_corr_tau_z_x2_same_sign.png")
