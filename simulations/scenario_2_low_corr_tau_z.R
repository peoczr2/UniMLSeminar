...
run_scenario_2_low_corr_tau_z <- function(repo_root, output_dir, n_train, n_test, num_trees, beat_penalty, target_share, seed, selected_methods = NULL, use_cache = TRUE) {
  set.seed(seed)
  ...
  cache_key <- sprintf(
    "scenario_2_low_corr_tau_z_seed-%s_ntrain-%s_ntest-%s_trees-%s_penalty-%s_share-%s",
    seed,
    n_train,
    n_test,
    num_trees,
  ...
  plot_dir <- scenario_plot_dir(output_dir, "scenario_2_low_corr_tau_z")
