run_scenario_1_high_corr_tau_z <- function(repo_root, output_dir, n_train, n_test, num_trees, beat_penalty, target_share, seed, methods_to_run = NULL) {
  set.seed(seed)
  ...
  if ("BTGQ" %in% result_names) {
    btgq_plot_path <- file.path(output_dir, sprintf("scenario_1_high_corr_tau_z_btgq_lambda_trace_seed_%s.png", seed))
    plot_btgq_lambda_trace(
      method_results[[which(result_names == "BTGQ")[1]]]$lambda_trace,
      btgq_plot_path,
      "Scenario 1 BTGQ: Lambda vs Targeted Group Demo",
      target_quota = 0.5
    )
  }

  if ("BTGQ_VALID" %in% result_names) {
    btgq_valid_plot_path <- file.path(output_dir, sprintf("scenario_1_high_corr_tau_z_btgq_valid_lambda_trace_seed_%s.png", seed))
    plot_btgq_lambda_trace(
      method_results[[which(result_names == "BTGQ_VALID")[1]]]$lambda_trace,
      btgq_valid_plot_path,
      "Scenario 1 BTGQ_VALID: Lambda vs Targeted Group Demo",
      target_quota = 0.5
    )
  }

  if ("BTGQ_MAX" %in% result_names) {
    btgq_max_plot_path <- file.path(output_dir, sprintf("scenario_1_high_corr_tau_z_btgq_max_lambda_trace_seed_%s.png", seed))
    plot_btgq_lambda_trace(
      method_results[[which(result_names == "BTGQ_MAX")[1]]]$lambda_trace,
      btgq_max_plot_path,
      "Scenario 1 BTGQ_MAX: Lambda vs Targeted Group Demo",
      target_quota = 0.5,
  ...
  metrics_df$scenario <- "scenario_1_high_corr_tau_z"

  if (length(method_results) > 0) {
    scores_df <- build_scores_frame(sim_data$test, method_results)
    plot_path <- file.path(output_dir, "scenario_1_high_corr_tau_z.png")
    plot_method_scores(scores_df, plot_path, "Scenario 1: High corr, tau = f(Z)")
  }
...
if (sys.nframe() == 0) {
  run_scenario_script(run_scenario_1_high_corr_tau_z, "scenario_1_high_corr_tau_z")
}
