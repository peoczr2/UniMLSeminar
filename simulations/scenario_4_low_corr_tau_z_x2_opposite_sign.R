...
run_scenario_4_low_corr_tau_z_x2_opposite_sign <- function(repo_root, output_dir, n_train, n_test, num_trees, beat_penalty, target_share, seed, selected_methods = NULL, use_cache = TRUE) {
  set.seed(seed)
  ...
  cache_key <- sprintf(
    "scenario_4_low_corr_tau_z_x2_opposite_sign_seed-%s_ntrain-%s_ntest-%s_trees-%s_penalty-%s_share-%s",
    seed,
    n_train,
    n_test,
    num_trees,
  ...
  plot_dir <- scenario_plot_dir(output_dir, "scenario_4_low_corr_tau_z_x2_opposite_sign")
  ...
  if (!is.null(btgq_dumb_result)) {
    ...
    plot_btgq_lambda_trace(
      ...
      btgq_dumb_plot_path,
      "Scenario 4 BTGQ_DUMB: Lambda vs Targeted Group Demo",
      target_quota = 0.5
    )
  }
