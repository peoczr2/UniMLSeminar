...
#include "BTGQDumbInstrumentalSplittingRule.h"

namespace grf {

namespace {

bool compute_child_concentration(double weight_sum,
                                 double target_group_sum,
                                 double& concentration) {
  if (std::abs(weight_sum) <= 1e-16) {
    return false;
  }

  concentration = target_group_sum / weight_sum;
  return true;
}

double compute_btgq_dumb_uplift(double weight_sum_left,
                                double target_group_sum_left,
                                double weight_sum_right,
                                double target_group_sum_right) {
  double concentration_left = 0.0;
  double concentration_right = 0.0;
  bool left_valid = compute_child_concentration(weight_sum_left, target_group_sum_left, concentration_left);
  bool right_valid = compute_child_concentration(weight_sum_right, target_group_sum_right, concentration_right);
  ...
  if (left_valid && !right_valid) {
    return concentration_left;
  }

  if (!left_valid && right_valid) {
    return concentration_right;
  }

  return std::max(concentration_left, concentration_right);
}

} // namespace

BTGQDumbInstrumentalSplittingRule::BTGQDumbInstrumentalSplittingRule(size_t max_num_unique_values,
                                                                     uint min_node_size,
                                                                     double alpha,
...
BTGQDumbInstrumentalSplittingRule::~BTGQDumbInstrumentalSplittingRule() {
  if (counter != nullptr) {
    delete[] counter;
  }
...
bool BTGQDumbInstrumentalSplittingRule::find_best_split(const Data& data,
                                                        size_t node,
...
void BTGQDumbInstrumentalSplittingRule::find_best_split_value(const Data& data,
                                                              size_t node,
                                                              size_t var,
                                                              size_t num_samples,
  ...
  for (bool send_left : {true, false}) {
    ...
    for (size_t i = 0; i < num_splits; ++i) {
      ...
      double uplift = compute_btgq_dumb_uplift(weight_sum_left,
